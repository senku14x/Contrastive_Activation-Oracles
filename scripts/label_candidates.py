...
def label_one(r, comp, catch, cap):
    ...
    if fam == "L":
        # ablation / load-bearing: bare givens must not yield a definite (non-gold) answer.
        load_bearing = (neutral_argmax == gold)
        r["ablation_verified_load_bearing"] = bool(load_bearing)
    else:
        load_bearing = True  # not the Family-P concept

    if not (competence_ok and load_bearing):
        reason = ("competence_fail(neutral_argmax=%s, p_gold=%.2f)" % (neutral_argmax, nd.get(gold, 0.0))
                  if not competence_ok else "sidesteppable(ablated->definite)")
        return "discard", "label=None;" + reason, None
